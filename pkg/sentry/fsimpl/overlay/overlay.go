...
func (fstype FilesystemType) GetFilesystem(ctx context.Context, vfsObj *vfs.VirtualFilesystem, creds *auth.Credentials, source string, opts vfs.GetFilesystemOptions) (*vfs.Filesystem, *vfs.Dentry, error) {
	...
	if upperPathname, ok := mopts["upperdir"]; ok {
		...
		// and trusted.overlay.* xattrs can be used as overlay upper
		// layers. For mounts created through the mount syscall (not
		// internal), restrict to tmpfs and gofer ("9p"). Gofer support
		// enables Docker overlay2 inside gVisor containers when the
		// rootfs overlay is disabled (--overlay2=none).
